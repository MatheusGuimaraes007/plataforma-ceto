                  ...
                  <td colspan="5" class="px-4 py-4 md:px-10">
                    <div class="space-y-3">
                      ...
                      <div v-for="passo in msg.mensagens_passos" :key="passo.id_passo" class="p-3 bg-white rounded-md border border-gray-200 shadow-sm">
                        ...
                        <template v-if="passo.url && ['imagem','video','documento','audio'].includes(passo.tipo_passo)">
                          <div v-if="passo.tipo_passo === 'imagem'" class="mt-2">
                            <img :src="passo.url" alt="imagem" class="max-w-full max-h-60" />
                          </div>
                          <div v-if="passo.tipo_passo === 'video'" class="mt-2">
                            <video controls class="max-w-full max-h-60" :src="passo.url"></video>
                          </div>
                          <div v-if="['documento','audio'].includes(passo.tipo_passo)" class="mt-2">
                            <a :href="passo.url" target="_blank" download class="text-blue-600 hover:underline">
                              Baixar {{ passo.tipo_passo }}
                            </a>
                            <div v-if="passo.tipo_passo==='audio'" class="mt-2">
                              <audio controls :src="passo.url" class="w-full"></audio>
                            </div>
                          </div>
                        </template>
                        <!-- fallback URL text for other cases -->
                        <p v-else-if="passo.url" class="mt-2 text-sm text-gray-500">
